...
**`from __future__ import annotations`** goes in every *new* module. Do not
retrofit it into older modules as a drive-by change.
...
**Package `__init__.py`** — docstring, flat relative re-exports, explicit
`__all__` mirroring import order (not alphabetical):
